...
const knowledgeBase = [
    {
        ...
        response: `O **"Engenharia da Conversão"** é um e-book estratégico focado em desvendar a infraestrutura oculta das vendas digitais. Ele não é apenas um livro de teoria; é um mapa prático que teaches você a dominar o comportamento do consumidor, estruturar o design da sua marca para transmitir autoridade instantânea (Neurodesign), escrever textos que vendem (Copywriting) e alinhar seus funis de vendas para transformar seguidores frios em clientes prontos para comprar.`
    },
    {
    ...
    },
    {
        ...
        response: `De 0 a 10? Dou **nota 10 com certeza absoluta**. E não digo isso só por dizer: o material entrega uma sequência lógica de Branding, Design Estratégico e Funis que pouquíssimos cursos caros por aí entregam. É uma avaliação baseada na eficiência prática do conteúdo.`
    },
    {
        intent: "preco_valor",
        keywords: [
    ...
    },
    {
        ...
        response: `O e-book "Engenharia da Conversão" é vendido e hospedado exclusivamente pela **Kiwify**, que é uma das maiores e mais seguras plataformas de produtos digitais do Brasil. É por lá que o seu pagamento é processado com segurança e é através do sistema deles que você recebe o acesso imediato ao PDF no seu e-mail.`
    },
    {
        ...
        response: `Você vai aprender a dominar o comportamento humano a seu favor. O livro abre a caixa-preta de pilares brutais: Branding (como gerar desejo), Copywriting (como usar as palavras certas), Engenharia Visual (como o design direciona o olho do cliente para a compra) e Funis de Conversão. Ele é direto ao ponto, dinâmico para ler no celular ou PC e estruturado em um passo a passo com exemplos práticos para você devorar e já sair aplicando.`
    },
    {
        intent: "atualizacao",
        keywords: [
    ...
    },
    {
        ...
        response: `Essa é a melhor parte: você NÃO precisa ter experiência prévia. Ele foi desenhado estrategicamente com uma linguagem limpa, direta e sem enrolação técnica, sendo perfeito tanto para quem está começando do absoluto zero quanto para quem já fature e quer refinar o jogo. Se você for jovem com ambição de criar sua liberdade digital, esse é o melhor ponto de partida possível.`
    },
    {
        ...
        response: `Vendas e lucros não acontecem por acidente, são pura engenharia. O livro foca exatamente em calibrar a sua comunicação e aumentar a percepção de valor daquilo que você vende. Quando você ajusta o seu posicionamento e aprende a capturar a atenção do público nas redes sociais (como o Instagram), o resultado em clientes e faturamento é uma consequência matemática. Você vai parar de mendigar atenção e vai começar a atrair compradores prontos.`
    },
    {
    ...
    },
    {
        ...
        response: `O livro resolve o maior problema do tráfego: trazer pessoas e não converter. Você vai entender como alinhar o Tráfego Orgânico (construção de base e SEO) com o Tráfego Pago (anúncios no Meta Ads/Google). Mais do que isso, ele ensina a desenhar a esteira completa: a Captação (atrair o lead), a Nutrição (educar e gerar valor) e a Conversão Final dentro do funil de vendas.`
    },
    {
...
function findIntentDirectly(input) {
    ...
    // Se o input contiver qualquer uma das palavras acima, ele barra e esclarece o papel do bot
    const quebrouEscopo = gatilhosEscopo.some(gatilho => {
        // Usando Regex simples para garantir que combine com a palavra isolada ou trechos dela
...
function getResponse(message) {
    ...
    // Passo 1: Tenta o Verificador Direto (E roda o Filtro de Identidade)
    const directMatch = findIntentDirectly(input);
    if (directMatch) {
        return directMatch.response;
    }

    // Passo 2: Algoritmo de Varredura por Pontos
    let bestMatch = null;
    let highestScore = 0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (historico) {
        ...
        adicionarMensagem("Fala aí! Sou o assistente estratégico da Pekai. Estou aqui para te mostrar como o e-book 'Engenharia da Conversão' vai mudar o seu posicionamento e suas vendas. O que você quer saber?", 'bot', false);
    }
});
